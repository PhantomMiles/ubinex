    ...
    <>
      ...
      <aside className={`fixed md:static left-0 top-0 w-64 h-screen bg-white border-r transition-transform duration-300 z-40 md:z-auto md:h-auto ${open ? 'translate-x-0' : '-translate-x-full md:translate-x-0'}`}>
        ...
        <nav className="p-4 space-y-1">
          <a href="#/admin/products" onClick={onClose} className={`block px-3 py-2 rounded ${active === 'products' ? 'bg-green-50 text-green-700' : 'hover:bg-gray-100'}`}>Products</a>
          <a href="#/admin/analytics" onClick={onClose} className={`block px-3 py-2 rounded ${active === 'analytics' ? 'bg-green-50 text-green-700' : 'hover:bg-gray-100'}`}>Analytics</a>
          <a href="#/" onClick={onClose} className="block px-3 py-2 rounded hover:bg-gray-100">Back to marketplace</a>
        </nav>
      </aside>
    </>
  );
