    ...
    <div className="min-h-screen bg-white font-sans">
      ...
      <main className="max-w-7xl mx-auto px-6 py-12">
        ...
        <div className="mb-12 flex flex-col md:flex-row md:items-end justify-between gap-8">
          <div>
            <p className="text-[10px] font-black text-gray-400 uppercase mb-3">Accounts / History</p>
          ...
          <div className="w-full md:w-80">
            <div className="relative">
              ...
              <input 
                ...
                className="w-full pl-12 pr-6 py-4 bg-gray-50 border border-gray-100 rounded-lg text-[10px] font-black uppercasest focus:ring-4 focus:ring-primary/5 transition shadow-inner outline-none"
              />
            </div>
          </div>
        </div>
        ...
        <div className="space-y-6">
          {filteredOrders.length > 0 ? (
                  ...
                  <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-8">
                    ...
                    <div className="flex-1">
                      <div className="flex items-center gap-4 mb-4">
                        <span className="text-[10px] font-black text-white bg-[#0a0a0a] px-3 py-1 rounded shadow-lg uppercasest">#{order.id}</span>
                        <span className={`text-[8px] font-black px-3 py-1 rounded border uppercase ${getStatusStyle(order.status)}`}>
                          {order.status}
                        </span>
                      </div>
                      <h3 className="text-2xl font-black text-gray-900 uppercase mb-2 leading-none">{order.productName}</h3>
                      <div className="flex flex-wrap gap-6 mt-4">
                         <p className="text-[10px] font-black text-gray-400 uppercasest flex items-center gap-2">
                    ...
                    <div className="flex items-center gap-8 lg:border-l lg:border-gray-100 lg:pl-12">
                       <div className="text-right">
                         <p className="text-[8px] font-black text-gray-400 uppercasest mb-1">Items Summary</p>
                         <p className="text-sm font-black text-gray-900">{order.quantity} {order.unit}</p>
                       </div>
                       <div className="text-right">
                         <p className="text-[8px] font-black text-gray-400 uppercasest mb-1">Amount Settled</p>
                ...
                <div className="px-8 pb-4">
                   <div className="flex justify-between mb-2">
                       {["Received", "Processing", "Shipped", "Delivered"].map((step, idx) => (
                         <div key={idx} className={`text-[8px] font-black uppercasest ${idx <= (order.status === 'delivered' ? 3 : order.status === 'in-transit' ? 2 : 1) ? 'text-primary' : 'text-gray-200'}`}>
                            {step}
                         </div>
                       ))}
                   </div>
            ...
            <div className="py-40 text-center bg-gray-50 rounded-xl border border-dashed border-gray-200 flex flex-col items-center justify-center">
               ...
               <h2 className="text-3xl font-black text-gray-900 uppercase mb-4">No records found</h2>
               <p className="text-[10px] text-gray-400 font-black uppercase">We couldn't find any orders matching your criteria</p>
            </div>
          )}
        </div>
        ...
        <div className="mt-20 p-12 bg-gray-50 rounded-xl border border-gray-100 flex flex-col md:flex-row justify-between items-center gap-8">
           <div className="flex items-center gap-6">
              ...
              <div>
                 <h4 className="text-xl font-black uppercase text-gray-900 leading-none mb-2">Need Assistance?</h4>
                 <p className="text-[10px] font-black text-gray-400 uppercasest leading-none">Our logistics support is active 24/7 for you</p>
              </div>
           </div>
           <button className="bg-[#0a0a0a] hover:bg-primary text-white px-10 py-5 rounded-lg text-[10px] font-black uppercase shadow-2xl transition-all duration-500 active:scale-95">Open Support Ticket</button>
        </div>
      </main>
    </div>
  );
