    ...
    <div className="h-[calc(100vh-140px)] flex bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden font-sans animate-in fade-in slide-in-from-bottom-6 duration-700">
      ...
      <div className="w-full md:w-80 border-r border-gray-100 flex flex-col">
        <div className="p-6 border-b border-gray-100">
          <h2 className="text-xl font-black uppercase text-gray-900">Market Hub</h2>
          <p className="text-[9px] font-black text-gray-400 uppercasest mt-1">Active Negotiations</p>
        </div>
          ...
          {negotiations.map(neg => (
             ...
             >
                <div className="flex justify-between items-start mb-2">
                   <span className="text-[10px] font-black uppercasest text-gray-900">{neg.buyerName}</span>
                   <span className="text-[8px] font-black text-gray-300 uppercase">2m ago</span>
                </div>
                <p className="text-[10px] text-gray-500 font-bold truncatest leading-tight">{neg.lastMessage}</p>
                <div className="mt-4 flex items-center gap-2">
                   <span className={`w-1.5 h-1.5 rounded-full ${neg.status === 'active' ? 'bg-green-500' : 'bg-orange-500'}`}></span>
                   <span className="text-[8px] font-black uppercasest text-gray-400">{neg.status} Inquiry</span>
                </div>
             </div>
          ))}
        </div>
      ...
      <div className="hidden md:flex flex-1 flex-col bg-gray-50/50">
        {selectedNeg ? (
          <>
            <div className="p-6 bg-white border-b border-gray-100 flex justify-between items-center">
               <div className="flex items-center gap-4">
                  ...
                  <div>
                    <h3 className="text-sm font-black text-gray-900 uppercasest">{selectedNeg.buyerName}</h3>
                    <p className="text-[8px] text-gray-400 font-black uppercasest">Inquiry for Enugu Yellow Pepper</p>
                  </div>
               </div>
               <button className="text-[9px] font-black text-primary uppercasest border-b border-primary">View Product Details</button>
            </div>
            ...
            <div className="flex-1 p-10 overflow-y-auto space-y-8 no-scrollbar">
               ...
               <div className="flex flex-col items-start max-w-[70%]">
                  ...
                  <span className="text-[7px] font-black text-gray-300 uppercasest mt-2 px-2">10:42 AM</span>
               </div>
               ...
               <div className="flex flex-col items-end max-w-[70%] ml-auto">
                  ...
                  <span className="text-[7px] font-black text-gray-300 uppercasest mt-2 px-2">Recently</span>
               </div>
            </div>
            ...
            <div className="p-8 bg-white border-t border-gray-100">
               <form onSubmit={handleSend} className="relative">
                  <input 
                    ...
                    className="w-full bg-gray-50 border border-gray-100 rounded-xl py-5 pl-8 pr-32 text-[10px] font-black uppercasest focus:ring-4 focus:ring-primary/5 transition shadow-inner" 
                  />
                  <button className="absolute right-3 top-1/2 -translate-y-1/2 bg-primary text-white px-6 py-2.5 rounded-lg text-[9px] font-black uppercasest shadow-lg hover:shadow-primary/20 transition-all active:scale-95">
                    Send Reply
                  </button>
               </form>
            </div>
          ...
          <div className="flex-1 flex flex-col items-center justify-center p-20 text-center opacity-20 grayscale">
             ...
             <h2 className="text-2xl font-black uppercase">No Active Chat</h2>
             <p className="text-[10px] font-black uppercase mt-2">Select a negotiation from the sidebar to begin responding.</p>
          </div>
        )}
      </div>
    </div>
