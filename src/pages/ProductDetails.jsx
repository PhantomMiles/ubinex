  ...
  return (
    <div className="bg-white rounded-2xl shadow-2xl max-w-2xl w-full max-h-[95vh] overflow-y-auto font-sans relative">
      ...
      <div className="sticky top-0 bg-white/80 backdrop-blur-md z-20 border-b border-gray-100 p-6 flex justify-between items-center">
        <div>
          <p className="text-[10px] font-black text-primary uppercase mb-1">Product Details</p>
          <h2 className="text-xl font-black text-gray-900 uppercase leading-none">{product.name}</h2>
        </div>
        <button
          onClick={onClose}
      ...
      <div className="p-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-12">
          ...
          <div className="space-y-8">
            <div>
              <p className="text-[10px] font-black text-gray-400 uppercasest mb-4">Origin & Quality</p>
              <div className="flex flex-col gap-3">
                <div className="flex items-center gap-3 bg-gray-50 px-4 py-3 rounded-lg border border-gray-100">
                  <i className="fas fa-map-marker-alt text-accent"></i>
                  <span className="text-[10px] font-black uppercasest text-gray-700">{product.location || "Enugu, Nigeria"}</span>
                </div>
                <div className="flex items-center gap-3 bg-gray-50 px-4 py-3 rounded-lg border border-gray-100">
                  <i className="fas fa-certificate text-primary"></i>
                  <span className="text-[10px] font-black uppercasest text-gray-700">Verified Grade A</span>
                </div>
              </div>
            </div>

            <div>
              <p className="text-[10px] font-black text-gray-400 uppercasest mb-4">Market Description</p>
              <p className="text-xs font-bold text-gray-600 leading-loose uppercase">
          ...
          <div className="bg-[#0a0a0a] text-white p-8 rounded-xl shadow-2xl relative overflow-hidden flex flex-col justify-between h-full">
            <div className="relative z-10">
               <p className="text-[10px] font-black text-primary uppercase mb-4">Consolidated Price</p>
               <div className="flex items-end gap-2 mb-8">
                  <span className="text-5xl font-black text-white line-height-none">{symbol}{price.toLocaleString()}</span>
                  <span className="text-xs font-black text-white/30 uppercasest mb-2">/ {unit}</span>
               </div>
               
               <div className="space-y-4 mb-10">
                  <div className="flex justify-between items-center text-[10px] font-black uppercasest text-white/40">
            ...
            <div className="space-y-4 relative z-10 mt-auto">
               <button 
                  ...
                  className="w-full bg-primary hover:bg-white hover:text-primary text-white font-black py-5 rounded-lg text-[10px] uppercase transition-all shadow-xl"
               >
                  Add to Shopping Bag
               </button>
               <button 
                  onClick={() => onNegotiate && onNegotiate()}
                  className="w-full bg-white/10 hover:bg-white/20 text-white font-black py-5 rounded-lg text-[10px] uppercase transition-all border border-white/10"
               >
                  Negotiate Price
               </button>
            </div>
        ...
        <div className="mt-12 pt-8 border-t border-gray-100 flex justify-between items-center grayscale opacity-30">
           ...
           <p className="text-[8px] font-black uppercasest text-gray-400">Copyright © 2026 Ubinex Limited</p>
        </div>
      </div>
    </div>
  );
