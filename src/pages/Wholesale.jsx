    ...
    <div className="min-h-screen bg-white font-sans">
      ...
      <main className="max-w-7xl mx-auto px-6 py-12">
        ...
        <div className="mb-16 flex flex-col md:flex-row justify-between items-end gap-8">
          <div>
            <p className="text-[10px] font-black text-primary uppercase mb-4">Bulk Procurement</p>
            <h1 className="text-5xl font-black text-gray-900 uppercase leading-none mb-4">
              Wholesale Market
            </h1>
            <p className="text-xs text-gray-400 font-bold uppercasest max-w-lg leading-relaxed">
              Unlock maximum value with bulk purchases directly from regional consolidation centers in Enugu and beyond.
            </p>
          </div>
          <button className="bg-[#0a0a0a] text-white px-12 py-5 rounded-lg text-[10px] font-black uppercasest hover:bg-primary transition shadow-2xl border border-white/5 active:scale-95">Download Price List</button>
        </div>

        {/* Categories Bar */}
        <div className="flex gap-4 mb-12 overflow-x-auto pb-4 no-scrollbar">
           {["All Bulk", "Bulk Grains", "Palm Oil", "Bulk Tubers", "Sacks"].map(cat => (
             <button key={cat} className="px-6 py-3 rounded-lg bg-gray-50 border border-gray-100 text-[9px] font-black uppercasest text-gray-500 hover:bg-primary hover:text-white hover:border-primary transition whitespace-nowrap">
               {cat}
             </button>
           ))}
        </div>
          ...
          {wholesaleOffers.map((offer) => (
            <div key={offer.name} className="group bg-white rounded-xl overflow-hidden border border-gray-200 hover:shadow-2xl transition duration-500 relative">
              <div className="h-56 overflow-hidden">
                ...
                <div className="absolute top-4 left-4 bg-primary text-white text-[8px] font-black uppercasest px-3 py-1.5 rounded shadow-lg">
                   {offer.category}
                </div>
              </div>
              <div className="p-10">
                <h3 className="text-xl font-black uppercase text-gray-900 mb-8">{offer.name}</h3>
                <div className="flex justify-between items-end pt-8 border-t border-gray-100">
                  <div>
                    <p className="text-[9px] font-black text-gray-400 uppercasest mb-1">Bulk Rate</p>
        ...
        <div className="mt-24 grid md:grid-cols-2 gap-12">
           <section className="p-12 bg-gray-50 rounded-xl border border-gray-100">
              <h2 className="text-3xl font-black uppercase text-gray-900 mb-6">Industrial Supply</h2>
              <p className="text-xs text-gray-600 font-bold leading-relaxed uppercasest mb-8">
              ...
              <button className="bg-primary text-white px-8 py-4 rounded-lg text-[9px] font-black uppercasest shadow-xl">Inquire Now</button>
           </section>
           <section className="p-12 bg-[#0a0a0a] text-white rounded-xl border border-white/5 relative overflow-hidden">
              ...
              <h2 className="text-3xl font-black uppercase text-white mb-6">Logistics for Bulk</h2>
              <p className="text-xs text-white/50 font-bold leading-relaxed uppercasest mb-8">
                 Dedicated freight services for orders above 2 tons. We handle the heavy lifting from the farm gate to your warehouse.
              </p>
              <button className="text-[10px] font-black uppercase text-primary border-b-2 border-primary pb-1">View Routes</button>
           </section>
        </div>
      </main>
    </div>
