  ...
  return (
    <div className="min-h-screen bg-white font-sans">
      ...
      <main className="max-w-7xl mx-auto px-6 py-12">
        {/* Header */}
        <div className="mb-20">
          <p className="text-[10px] font-black text-primary uppercase mb-4">Agricultural Services</p>
          <h1 className="text-5xl font-black text-gray-900 uppercase leading-none mb-6">
            Producers & Partners
          </h1>
          <p className="text-xs text-gray-400 font-bold uppercasest max-w-2xl leading-relaxed">
        ...
        <div className="flex gap-4 mb-16 overflow-x-auto pb-4 no-scrollbar">
           {["All Services", "Verified Farms", "Farming Tools", "Seedlings", "Consultancy"].map(cat => (
             <button key={cat} className="px-8 py-4 rounded-lg bg-gray-50 border border-gray-100 text-[10px] font-black uppercasest text-gray-500 hover:bg-[#0a0a0a] hover:text-white hover:border-[#0a0a0a] transition whitespace-nowrap shadow-sm">
               {cat}
             </button>
           ))}
        </div>
          ...
          {registeredFarms.map((farm) => (
            <div key={farm.name} className="group bg-white rounded-xl overflow-hidden border border-gray-100 hover:shadow-2xl transition duration-500">
               <div className="relative h-64">
                  ...
                  <div className="absolute top-4 right-4 bg-white/90 backdrop-blur-md px-4 py-2 rounded-lg shadow-xl text-[8px] font-black uppercasest text-primary flex items-center gap-2">
                     <i className="fas fa-certificate text-xs"></i> {farm.type}
                  </div>
               </div>
               <div className="p-10">
                  <h3 className="text-xl font-black uppercase text-gray-900 mb-1">{farm.name}</h3>
                  <p className="text-[9px] font-black uppercasest text-gray-400 mb-6">Lead: {farm.owner}</p>
                  
                  <div className="flex items-center gap-2 mb-8 bg-gray-50 w-fit px-4 py-2 rounded-lg border border-gray-100">
                     <span className="text-[9px] font-black uppercasest text-gray-700">{farm.specialty}</span>
                  </div>

                  <div className="pt-8 border-t border-gray-100 flex justify-between items-center">
                     <div className="flex items-center gap-2">
                        <i className="fas fa-map-marker-alt text-primary text-xs"></i>
                        <span className="text-[9px] font-black uppercasest text-gray-500">{farm.location}</span>
                     </div>
                     <button className="text-[10px] font-black uppercasest text-primary border-b-2 border-primary pb-0.5">Contact</button>
                  </div>
               </div>
            </div>
          ))}
        ...
        <section className="mt-24 p-16 bg-[#0a0a0a] rounded-xl text-center relative overflow-hidden border border-white/5">
           ...
           <div className="relative z-10 max-w-xl mx-auto">
              <h2 className="text-4xl font-black uppercase text-white mb-8">Join the Directory</h2>
              <p className="text-[10px] font-bold text-white/40 uppercasest mb-10 leading-loose mx-auto">
              ...
              <button className="bg-primary hover:bg-white hover:text-primary text-white font-black px-12 py-5 rounded-lg text-[10px] uppercase transition-all shadow-xl active:scale-95 border border-white/10">Become a Partner</button>
           </div>
        </section>
      </main>
    </div>
