...
export default function Settings() {
  ...
  const Content = (
    <div className="max-w-4xl animate-in fade-in slide-in-from-bottom-6 duration-700 font-sans">
      <div className="mb-12">
        <p className="text-[10px] font-black text-primary uppercase mb-4">Account Control</p>
        <h1 className="text-4xl font-black text-gray-900 uppercase leading-none mb-4">
          Profile Settings
        </h1>
        <p className="text-xs text-gray-400 font-bold uppercasest">
      ...
      <form onSubmit={handleSubmit} className="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
         <div className="p-10 space-y-10">
            <div className="grid md:grid-cols-2 gap-10">
               <div className="space-y-3">
                  <label className="text-[10px] font-black uppercasest text-gray-400">Full Name</label>
                  <input 
                    type="text" 
                    value={formData.name}
                    onChange={e => setFormData({...formData, name: e.target.value})}
                    className="w-full bg-gray-50 border border-gray-100 rounded-lg py-4 px-6 text-[11px] font-black uppercasest focus:ring-4 focus:ring-primary/5 transition" 
                  />
               </div>
               <div className="space-y-3">
                  <label className="text-[10px] font-black uppercasest text-gray-400">Phone Number</label>
                  <input 
                    type="tel" 
                    placeholder="+234..."
               ...
               <div className="space-y-3">
                  <label className="text-[10px] font-black uppercasest text-gray-400">Date of Birth</label>
                  <input 
                    type="date" 
                    value={formData.dob}
                    onChange={e => setFormData({...formData, dob: e.target.value})}
                    className="w-full bg-gray-50 border border-gray-100 rounded-lg py-4 px-6 text-[11px] font-black uppercasest focus:ring-4 focus:ring-primary/5 transition cursor-pointer" 
                  />
               </div>
               <div className="space-y-3">
                  <label className="text-[10px] font-black uppercasest text-gray-400">Regional Location</label>
                  <input 
                    type="text" 
            ...
            <div className="pt-10 border-t border-gray-50">
               <div className="bg-gray-50 p-6 rounded-xl border border-gray-100 flex items-center justify-between">
                  <div>
                    <p className="text-[10px] font-black uppercasest text-gray-900 mb-1">Ubinex Identity</p>
                    <p className="text-[9px] font-black text-primary uppercase">{user?.ubxId || 'Assigned on next login'}</p>
                  </div>
                  <div className="px-4 py-2 bg-white rounded-lg border border-gray-100 text-[8px] font-black uppercasest text-gray-400">
                    ID Non-Editable
                  </div>
               </div>
            </div>
         ...
         <div className="p-8 bg-gray-50 border-t border-gray-100 flex justify-end gap-6">
            {saved && (
              <span className="flex items-center gap-2 text-green-600 text-[10px] font-black uppercasest animate-in fade-in slide-in-from-right-4">
                <i className="fas fa-check-circle"></i> Changes Committed
              </span>
            )}
            <button className="bg-primary hover:bg-[#0a0a0a] text-white px-12 py-4 rounded-lg text-[10px] font-black uppercase transition-all shadow-xl active:scale-95">
               Update Profile
            </button>
         </div>
      </form>
      ...
      {user?.role === 'consumer' && (
        <div className="mt-16 bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden animate-in slide-in-from-bottom-6 duration-700">
           <div className="p-10 border-b border-gray-100">
              <p className="text-[10px] font-black text-primary uppercase mb-2">Regional Wallet</p>
              <h3 className="text-xl font-black text-gray-900 uppercase">Payment Methods</h3>
           </div>
           <div className="p-10 grid md:grid-cols-2 gap-12 items-start">
              <div className="space-y-8">
                 <div className="p-8 rounded-xl border-2 border-primary/20 bg-primary/5 relative overflow-hidden group">
                    ...
                    <p className="text-[9px] font-black uppercasest text-primary mb-6">Default Naira Card</p>
                    <div className="text-xl font-black text-gray-900st mb-2">**** **** **** 4242</div>
                 ...
                 <button className="w-full py-4 border-2 border-dashed border-gray-200 rounded-xl text-[10px] font-black uppercasest text-gray-400 hover:border-primary hover:text-primary transition">
                    + Link New Payment Gateway
                 </button>
              </div>

              <div className="bg-gray-50 p-8 rounded-xl border border-gray-100 space-y-6">
                 <p className="text-[9px] font-black uppercasest text-gray-400">Card Entry Point</p>
                 ...
                 <button className="w-full bg-[#0a0a0a] text-white py-4 rounded-lg text-[10px] font-black uppercasest shadow-xl hover:bg-primary transition active:scale-95">Verify & Add Card</button>
              </div>
           </div>
        </div>
      )}
      ...
      <div className="mt-12 grid md:grid-cols-2 gap-8">
         <div onClick={() => setShowSecurity(true)} className="p-8 bg-white border border-gray-100 rounded-xl relative group cursor-pointer hover:border-primary transition">
            <h4 className="text-[10px] font-black uppercasest text-gray-900 mb-4">Security</h4>
            <p className="text-[9px] text-gray-400 font-bold uppercasest">Update your password and login credentials.</p>
         ...
         <div onClick={() => setShowDelete(true)} className="p-8 bg-white border border-gray-100 rounded-xl relative group cursor-pointer hover:border-red-100 transition">
            <h4 className="text-[10px] font-black uppercasest text-red-600 mb-4">Deactivation</h4>
            <p className="text-[9px] text-gray-400 font-bold uppercasest">Close your account and purge archived data.</p>
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-6 bg-black/80 backdrop-blur-md animate-in fade-in duration-300">
           <div className="bg-white w-full max-w-sm rounded-2xl overflow-hidden shadow-2xl animate-in zoom-in-95 duration-500">
              <div className="p-10 text-center">
                 ...
                 <h3 className="text-xl font-black text-gray-900 uppercase mb-4">Close Account?</h3>
                 <p className="text-[10px] font-bold text-gray-400 uppercasest leading-relaxed mb-10">
                    This will permanently delete your harvest statistics and transaction history.
                 </p>
                 <div className="flex flex-col gap-4">
                    <button onClick={deleteUserAccount} className="w-full bg-red-600 text-white py-4 rounded-lg text-[10px] font-black uppercasest shadow-xl hover:bg-red-700 transition active:scale-95">Terminate Identity</button>
                    <button onClick={() => setShowDelete(false)} className="w-full bg-gray-100 text-gray-600 py-4 rounded-lg text-[10px] font-black uppercasest hover:bg-gray-200 transition">Keep Account</button>
                 </div>
              </div>
           </div>
        </div>
    ...
    <div className="min-h-screen bg-white">
      ...
      <div className="max-w-7xl mx-auto px-10 py-20 flex gap-20">
         <div className="hidden lg:block w-64 space-y-2">
            <button className="w-full text-left px-6 py-4 rounded-xl bg-primary text-white text-[10px] font-black uppercasest shadow-xl">Profile Details</button>
            <button className="w-full text-left px-6 py-4 rounded-xl text-gray-400 hover:bg-gray-50 text-[10px] font-black uppercasest transition">Order History</button>
            <button className="w-full text-left px-6 py-4 rounded-xl text-gray-400 hover:bg-gray-50 text-[10px] font-black uppercasest transition">Security</button>
         </div>
