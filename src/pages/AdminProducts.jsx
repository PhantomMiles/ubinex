...
export default function AdminProducts() {
  ...
  const remove = (id) => {
    if (window.confirm("Confirm deletion of this product from your inventory?")) {
      setProducts(products.filter(p => p.id !== id));
    }
  };

  const categories = ["Vegetables", "Fruits", "Grains", "Roots", "Oils", "Spices", "Processed"];
  const units = ["kg", "basket", "tuber", "liter", "bottle", "piece", "bag"];
    ...
    <div className="animate-in fade-in slide-in-from-bottom-6 duration-700 font-sans">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-12 gap-8">
        <div>
          ...
          <h2 className="text-4xl font-black italic text-gray-900 leading-none uppercase tracking-tighter">My Products</h2>
        </div>
        <button 
      ...
      {showAddForm && (
        <form onSubmit={addProduct} className="bg-white p-10 rounded-xl shadow-2xl mb-16 border border-gray-100 animate-in zoom-in-95 duration-500 relative overflow-hidden">
          ...
          <h3 className="text-xl font-black italic mb-10 text-gray-900 uppercase tracking-tighter">Enter Product Details</h3>
          <div className="grid gap-10 md:grid-cols-2 lg:grid-cols-3">
            ...
            <div className="flex items-center gap-4 mt-8 bg-gray-50 p-4 rounded-lg border border-gray-100 italic">
          ...
          <div className="mt-12 flex justify-end">
             <button className="bg-primary hover:bg-[#0a0a0a] text-white font-black py-5 px-16 rounded-lg transition-all shadow-2xl text-[10px] uppercase tracking-[0.3em] italic transform active:scale-95">Commit harvest to Market</button>
          </div>
        </form>
      )}
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="bg-gray-50/50 border-b border-gray-100">
                <th className="px-10 py-6 text-[9px] font-black uppercase tracking-[0.3em] text-gray-400 italic">Produce Item</th>
                <th className="px-10 py-6 text-[9px] font-black uppercase tracking-[0.3em] text-gray-400 italic">Category</th>
                <th className="px-10 py-6 text-[9px] font-black uppercase tracking-[0.3em] text-gray-400 italic">Market Price</th>
                <th className="px-10 py-6 text-[9px] font-black uppercase tracking-[0.3em] text-gray-400 italic">Inventory Status</th>
                <th className="px-10 py-6 text-[9px] font-black uppercase tracking-[0.3em] text-gray-400 italic">Management</th>
              </tr>
            </thead>
                ...
                <tr key={p.id} className="hover:bg-gray-50/50 transition duration-300 group">
                  <td className="px-10 py-8">
                    <div className="flex items-center gap-5">
                       <div className="w-16 h-16 rounded-lg bg-gray-50 overflow-hidden border border-gray-100 flex items-center justify-center text-primary group-hover:scale-110 transition duration-500 shadow-inner italic font-black text-2xl">
                          {p.image ? <img src={p.image} className="w-full h-full object-cover" /> : <i className="fas fa-leaf scale-90 opacity-40"></i>}
                       </div>
                       <div>
                         <div className="text-sm font-black text-gray-900 uppercase italic tracking-tighter mb-1">{p.name}</div>
                  ...
                  <td className="px-10 py-8">
                    <span className="text-[9px] font-black uppercase tracking-[0.2em] px-4 py-1.5 bg-primary/5 text-primary rounded border border-primary/10 italic">
                      {p.category}
                    </span>
                  </td>
                  <td className="px-10 py-8">
                    <div className="text-lg font-black text-gray-900 italic">₦{p.price.toLocaleString()} <span className="text-[8px] text-gray-300 not-italic font-black uppercase tracking-widest ml-1">/ {p.unit}</span></div>
                  </td>
                  <td className="px-10 py-8">
                    <button 
                  ...
                  <td className="px-10 py-8">
                    <div className="flex items-center gap-4">
                       <button className="w-12 h-12 rounded-lg bg-white border border-gray-100 text-gray-300 hover:bg-primary hover:text-white hover:border-primary transition shadow-sm flex items-center justify-center group/btn active:scale-90 italic">
                          <i className="fas fa-edit text-xs"></i>
                       </button>
                       <button onClick={() => remove(p.id)} className="w-12 h-12 rounded-lg bg-white border border-gray-100 text-gray-300 hover:bg-red-600 hover:text-white hover:border-red-600 transition shadow-sm flex items-center justify-center group/btn active:scale-90">
          ...
          {products.length === 0 && (
            <div className="text-center py-40 px-10 bg-gray-50/50">
               ...
               <p className="text-[10px] font-black text-gray-400 uppercase tracking-[0.4em] max-w-sm mx-auto leading-relaxed italic">Your farm repository is currently empty. Please register your first harvest using the button above.</p>
            </div>
          )}
        </div>
      </div>
